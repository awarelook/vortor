...
python results/verify/verify_all.py      # 58/58 PASS, exit 0 iff all pass
```

`verify_all.py` globs every `results/verify/*.py` (55 theory scripts) + the three engine files
(`engine/ftgb_engine.py`, `engine/ftgb_synthesis_modeler.py`, `engine/ftgb_resonator_sim.py`) =
**58 checks**. This reproduces the entire load-bearing model: the CK spectrum, R2/R3 fluid theorems,
