...
python results/verify/verify_all.py      # 65/65 PASS, exit 0 iff all pass
```

`verify_all.py` globs every `results/verify/*.py` (62 theory scripts) + the three engine files
(`engine/ftgb_engine.py`, `engine/ftgb_synthesis_modeler.py`, `engine/ftgb_resonator_sim.py`) =
**65 checks**. This reproduces the entire load-bearing model: the CK spectrum, R2/R3 fluid theorems,
