...
(`verify_all.py` **85/85**). It asserts no number that is not traceable to an existing `[V]`/`[credited]`
...
`[V]`/`[credited]`; FTGB's own distinctive mechanisms (the anapole coherent **active site**, the B=4 Landau–Zener
...
- **Direct phonon-nuclear coupling dead ~66 OOM** → *forces* any surviving disposal onto the collective
  channel — the reason the `[S]` collective-disposal channel is even on the table.
...
`disposal_nonpopulation_check.py`, `disposal_coherence_volume_nogo_check.py`.
