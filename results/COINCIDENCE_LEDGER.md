...
| gate x density-proxy vs the rho_eff target band | `1/9 x [0.55,0.96] = [0.061,0.107]` | `0.06-0.08` | overlapping | **ELEVATED to a structured estimate (Stage E, 2026-09-15)** -- was logged coincidence-class, but the `1/9` is NOT a coincidence: it is the angular-momentum-forced ORIENTATION factor (4He 0+ <- singlet S=0; the spin gate), so `rho_eff = f_orient(1/9-1/3) x f_density([0.55,0.96]) x f_dyn` is a semi-analytic factorization, hitting the target for an O(1) dynamical factor (`delta_b4_stageE_rho_eff_semianalytic_check`). Still NOT the production number (f_dyn is the named O(1) external residual); orientation SELECTION rigorous `[credited]`, magnitudes carry the convention/proxy bands. |
| deuteron-Larmor / carrier-beat resonance density `n*` | `1.02e20 /m³` | cited SSPX range `1e19–1.3e20` | inside | **PREDICTION (v_A-independent)** — `f_Larmor(B(n)) = f_beat` reduces to a geometry+density condition (v_A cancels in the ratio), solved at `n* = 1.02e20 /m³`, inside the object's own cited ion-density range (`beat_spin_magnetic_resonance_check`, 2026-09-15). This is the magnetic-resonance condition for the beat→deuteron-spin coupling (mechanism [credited]); logged as a **density-tuned falsifier** (the effect peaks near n\*), NOT promoted. Caveat: the individual absolute `B` and `f_L` carry the v_A band; their ratio `n*` does not. |
