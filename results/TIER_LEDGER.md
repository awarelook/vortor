...
| **Cross-scale Δ identity** (kHz detuning ↔ MeV gap) | `[S]` / **pump-reading settled-neg** | Show the plasma detuning literally sets the nuclear gap. `[S]` hypothesis; only the plasma beat is `[V]`. **Sharpened (M16, `rhythm_parametric_resonance_check` TEST 4, arithmetic `[V]`):** the carrier-comb beat `ω_b=2π·87 kHz≈5.47×10⁵ rad/s` is a **difference** frequency and **cannot** parametrically pump or directly drive a MeV nuclear mode (`ω_nuc≈1.52×10²¹ rad/s`): parametric onset needs a **sum**/twice-frequency match `Ω≈2ω_nuc≈3.04×10²¹ rad/s` — short by `1.8×10⁻¹⁶` (15.7 OOM); the only tongue it hits, `n≈5.6×10¹⁵`, has exponentially vanishing width (adiabatic, not resonant). This **retires the "beat dynamically pumps the nucleus" reading only.** It does **NOT** touch (i) the conserved `d+d→⁴He=23.847 MeV` energy ledger (`[V]`, separate), (ii) the cold **Landau-Zener B=4** resolution, or (iii) a genuinely-slow (`ω_b≈ω_slow`) nuclear-adjacent collective/level-splitting route — the sole survivable cross-scale form is a **structural detuning-gap (Landau-Zener) analogy, never a dynamical pump.** |
...
*Provenance: the 66 `results/verify/` scripts + engine + the synthesis modeler + the resonator simulation `engine/ftgb_resonator_sim.py` (`verify_all.py` → **69/69** PASS) + the `results/` and `handoffs/` docs
