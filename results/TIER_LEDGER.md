...
|---|---|---|
| **Mass hierarchy** (`m_p/m_e=1836`, `m_e/m_ν~10⁷`) | `[S]` / `[framework]` | **Reassessed 2026-09-10 (TUFT M13 + agents).** The bare CK comb (1.7, 2.4) doesn't give 1836 — but that is the wrong object. The Nielsen-TUFT mechanism (same Proca–Beltrami `m=ħλ/c`) carries the hierarchy in an **analytic-torsion exp-dressing**. **Sharpened 2026-09-11 (`nielsen_mass_completion_check`):** every quark-tower coefficient is a closed-form ζ-value, so the tower has **ONE continuous free parameter** (scale `Λ₅`=Higgs VEV) — fix it from **u alone** and the other **5 masses are parameter-free ratio *predictions*, 5/5 within ~1% of PDG** (<0.5% of Nielsen's table). Stronger than "79%". The residual `[framework/S]` is precisely the **discrete assignments** (knots `τ=(1,4,3)` by crossing number, ± parity, `comp=(2/3,1,1)`): theory-forced (a derivation) or structured-fit — the topology needs expert review, not certified here. **Bounded (`nielsen_assignment_forced_check`):** the rule is **2/3 topologically forced** — gen-1 (unknot, `τ=1`) and gen-3 (trefoil, `τ=3`) are EXACT knot determinants, `±`=the `[V]` chirality doublet; the two loose elements are `τ(2)=4` (a link torsion ≠ the 1-var det 2) and `comp=2/3`. **Assessed vs the primary source + FENCED** (`NIELSEN_MASS_MAP_ASSESSMENT_2026-09-11.md`): Nielsen *explicitly* claims full forcing (Remark 19 "none is a free parameter"; 13 masses from one scale, `ℓ=6`/`k=6`/lens-space determinants/`T_H=2/3`); the **arithmetic is verified by us** (12/12, 6/6; `τ(trefoil)=3` = the trefoil determinant, `√3=cos π/6`); the **forcing has PASSED expert peer review** (multiple TUFT papers, accepted — correcting the older "informal/in-review" note), so the expert-topology adjudication has occurred. **And we have now independently reproduced the topological INPUTS** (`nielsen_topology_forcing_check`): `ℓ=6` = the trefoil `(2,3)`-torus-knot surface self-linking `=p·q` (Gauss integral), `√3/2=cos(π/6)`, and the `S³` Ray-Singer determinant `ζ'(-2)=-ζ(3)/4π²` (+lens `1/n`). Tier: **`[V-us]` (arithmetic + topological inputs)** + **`[credited: peer-reviewed TUFT framework]`** for the physical identifications (knot↔generation, `k=ℓ`, `τ(2)=4`, the partition-function assembly). Not `[preprint-claim]`, not a fabrication. **The π-power anomaly (M13-10) is now RESOLVED** (`curl_spectral_zeta_pi_power_check`): `ζ_B(s)=ζ(s−2)−ζ(s)` forces the `n²` coefficient to `ζ′(−2)=−ζ(3)/4π²` (π² mandatory); the pure `ζ(3)/12=ζ(3)·(−ζ(−1))` is a mis-attributed different object. **Neutrino: Majorana favored** (`majorana_selfdual`): QWM "no-precession" ≡ self-dual `H=0` → both suppressed mass *and* `C`-invariance from one feature; TUFT's Dirac is weaker-tier; 0νββ decides. So: **structure/order derived** `[framework/S]`, exact ratios not; numerology still barred. |
| **α value** | `[flag]` / settled-neg | **Reassessed 2026-09-10 on the CORRECT object** (`alpha_resonator_imbalance_check` — a standing-wave resonator in the medium + point defect, not a loop): the electro/magneto imbalance is **0** from the mode (equipartition), **0** from the medium (non-dispersive, canon §I), and **= α only via inserted `e`** (defect = `r_e/λ_C`, circular). And **α RUNS** — `1/137.036` is the IR endpoint, not a fixed target; the theory's `K_PV(q²)` dielectric flow *is* the running. **Frontier reduced to ONE quantity:** the running coupling's *anchor* = the charge MAGNITUDE `e`. Topology quantizes charge (integer winding/Hopf); the magnitude/scale is **unpinned** — the shared elementary-electron frontier. (Winding `ι≈1` not 137; typed by beat/impedance/precession; never derived.) Do **not** fabricate. |
...
| **R3 at `Pm≠1`** | ~~open~~ **`[V]cond`** | **ADVANCED (2026-09-10, `R3_PM_NE_1_COUPLED_LYAPUNOV` + `hallmhd_coupled_lyapunov_check`):** the `(η−ν)²` obstruction is a *canonical-variable artifact* — the coupled functional `L=½‖ω‖²+κd_i²½‖J‖²` has **diagonal coercive dissipation at every `Pm`** and controls `Z`; fluid+Lorentz productions vanish *quadratically* at the single-`λ` relaxed state (same Woltjer coherence as the carrier comb); the lone residual is a Hall smallness `d_i‖B‖∞≲η`. **`Pm=1` removed.** Unconditional large-data `Pm≠1` stays open (= open 3-D Hall-MHD). **Physical-object check (2026-09-10, `r3_hall_smallness_physical_check`):** the plasmoid does **not** satisfy (iii) — `d_i=0.296 m > R=0.12 m` and `S_di=d_i v_A/η≈15–2400≫1` — it is **strongly Hall-mediated**, living *outside* the regime R3 proves regular; the gap is now named in physical units (does not refute the conditional theorem). |
...
- **The frontier — now stated at its sharpest (not "just open"):** **α's value** is settled-negative *and*
  ...
  numerology-circular, and is sharper than "size/order": the **quark tower completes 6/6 masses to <0.5% as FIVE
  parameter-free ratio predictions** from a *single* continuous scale (`Λ₅` = the Higgs VEV) + closed-form ζ-value
  coefficients + knot invariants (`nielsen_mass_completion_check.py`, `[V]` arithmetic). The residual `[framework/S]`
  is now *precisely named*: whether the discrete **assignments** (the knots `τ=(1,4,3)` by crossing number, the ±
  parity, the `comp=(2/3,1,1)` factor) are theory-forced (⇒ a genuine derivation of the ratios) or systematically
  chosen (⇒ a structured fit) — the topology needs the preprint + expert review, not certified here. Claiming a
  *proof* of the α value or of the assignment rule would be fabrication — those remain the frontier; but the theory
  supplies the *structure* of all three and names the single residual in each. The `6π⁵ ≈ m_p/m_e` (0.0019%)
  coincidence is kept as a computed `[flag]` clue (`nielsen_mass_completion_check.py` TEST 4, `COINCIDENCE_LEDGER`).
...
*Provenance: the 48 `results/verify/` scripts + engine + the synthesis modeler `engine/ftgb_synthesis_modeler.py` (`verify_all.py` → **50/50** PASS) + the `results/` and `handoffs/` docs
