...
| **PV-gravity: no propulsion/levitation** | **`settled-neg`** | **NEW 2026-09-15** (`pv_gravity_no_propulsion_check`): the M11 polarizable-vacuum reading `K_PV=exp(2GM/rc²)` [credited: Puthoff] is GR-equivalent (solar redshift 2.12e-6 reproduced) and attractive for all positive mass; a levitating well needs negative mass-energy ⇒ **no levitation/anti-gravity/reactionless propulsion/over-unity** from the object's own positive field energy. The firewall's central fence, now computed (convention-robust). Storti graviton-propulsion [EGM] not adopted. |
| **The spin/orientation gate** | **`[credited]`-arith / `[S]` id / `[flag]` consistency** | **NEW 2026-09-15** (`spin_channel_gate_check`): the quantum arithmetic of Stage D's orientation space — singlet-only s-wave door (statistical gate 1/9); **steerable**: condensate pair m=0 → 1/3 (3×), m=±1 → 0 exactly (d-wave residual ~1e-6); thermal polarization ~1e-6 ⇒ selection must be dynamical/coherent (the Bose seed's mechanism-shaped job). Consistency `1/9×[0.55,0.96]=[0.061,0.107]` ∋ target band — **logged coincidence-class**, not promoted. **New falsifier: polarization-steered aneutronic yield.** |
...
- **The frontier, at its sharpest:** **α's value** is settled-negative *and* reframed — α **runs** (1/137.036 is
  ...
  lock itself stays `[S]`). The **mass hierarchy** is not numerology-circular: the **quark tower completes all six
  masses as FIVE parameter-free ratio predictions (5/5 within ~1% PDG, <0.5% of Nielsen's table)** from a *single* continuous scale (the Higgs VEV) +
...
*Provenance: the 82 `results/verify/` scripts + engine + the synthesis modeler + the resonator simulation `engine/ftgb_resonator_sim.py` (`verify_all.py` → **85/85** PASS) + the `results/` and `handoffs/` docs
