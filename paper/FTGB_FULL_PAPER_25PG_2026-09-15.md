...
Reproduce everything: `pip install -r requirements.txt && python results/verify/verify_all.py` → **86/86** PASS, deterministic, offline, CI-gated on every push.
...
*Plain language: the labels are the instrument. A reader who trusts nothing can re-run all 86 checks with one
...
1. **Every load-bearing claim has a script.** The harness (`verify_all.py`) runs 83 theory scripts plus three
