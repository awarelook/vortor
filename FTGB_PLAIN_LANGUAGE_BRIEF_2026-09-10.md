...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **62/62 PASS**.
...
All of it re-runs from one command (**62/62 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (62/62). Depth:
