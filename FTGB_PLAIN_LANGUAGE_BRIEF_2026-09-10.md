...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **67/67 PASS**.
...
All of it re-runs from one command (**67/67 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (67/67). Depth:
