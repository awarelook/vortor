...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **90/90 PASS**.
...
All of it re-runs from one command (**90/90 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (90/90). Depth:
