...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **73/73 PASS**.
...
All of it re-runs from one command (**73/73 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (73/73). Depth:
