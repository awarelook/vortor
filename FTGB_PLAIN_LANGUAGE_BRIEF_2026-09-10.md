...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **49/49 PASS**.
...
All of it re-runs from one command (**49/49 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (49/49). Depth:
