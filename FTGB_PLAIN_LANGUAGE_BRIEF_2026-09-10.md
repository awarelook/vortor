...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **52/52 PASS**.
...
All of it re-runs from one command (**52/52 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (52/52). Depth:
