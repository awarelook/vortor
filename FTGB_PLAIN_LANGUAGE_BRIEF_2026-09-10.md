...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **42/42 PASS**.
...
All of it re-runs from one command (**42/42 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (42/42). Depth:
