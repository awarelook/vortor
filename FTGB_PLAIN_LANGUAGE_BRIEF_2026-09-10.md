...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **88/88 PASS**.
...
All of it re-runs from one command (**88/88 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (88/88). Depth:
