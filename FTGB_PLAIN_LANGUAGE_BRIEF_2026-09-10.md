...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **60/60 PASS**.
...
All of it re-runs from one command (**60/60 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (60/60). Depth:
