...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **63/63 PASS**.
...
All of it re-runs from one command (**63/63 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (63/63). Depth:
