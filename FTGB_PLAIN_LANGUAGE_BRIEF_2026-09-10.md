...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **46/46 PASS**.
...
All of it re-runs from one command (**46/46 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (46/46). Depth:
