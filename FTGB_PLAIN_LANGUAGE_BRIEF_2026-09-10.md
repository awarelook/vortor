...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **74/74 PASS**.
...
All of it re-runs from one command (**74/74 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (74/74). Depth:
