...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **44/44 PASS**.
...
All of it re-runs from one command (**44/44 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (44/44). Depth:
