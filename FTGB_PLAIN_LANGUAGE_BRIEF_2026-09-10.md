...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **75/75 PASS**.
...
All of it re-runs from one command (**75/75 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (75/75). Depth:
