...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **56/56 PASS**.
...
All of it re-runs from one command (**56/56 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (56/56). Depth:
