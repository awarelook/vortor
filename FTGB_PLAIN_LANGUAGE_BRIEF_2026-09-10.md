...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **83/83 PASS**.
...
All of it re-runs from one command (**83/83 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (83/83). Depth:
