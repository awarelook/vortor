...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **50/50 PASS**.
...
All of it re-runs from one command (**50/50 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (50/50). Depth:
