...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **43/43 PASS**.
...
All of it re-runs from one command (**43/43 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (43/43). Depth:
