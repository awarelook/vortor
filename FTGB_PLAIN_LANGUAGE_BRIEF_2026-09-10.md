...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **70/70 PASS**.
...
All of it re-runs from one command (**70/70 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (70/70). Depth:
