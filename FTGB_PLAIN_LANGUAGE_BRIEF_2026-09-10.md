...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **48/48 PASS**.
...
All of it re-runs from one command (**48/48 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (48/48). Depth:
