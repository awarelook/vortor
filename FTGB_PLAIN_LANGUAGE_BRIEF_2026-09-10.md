...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **82/82 PASS**.
...
All of it re-runs from one command (**82/82 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (82/82). Depth:
