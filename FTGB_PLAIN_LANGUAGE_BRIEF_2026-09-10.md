...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **61/61 PASS**.
...
All of it re-runs from one command (**61/61 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (61/61). Depth:
