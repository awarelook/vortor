...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **92/92 PASS**.
...
All of it re-runs from one command (**92/92 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (92/92). Depth:
