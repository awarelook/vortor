...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **39/39 PASS**.
...
All of it re-runs from one command (**39/39 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (39/39). Depth:
