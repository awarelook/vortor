...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **91/91 PASS**.
...
All of it re-runs from one command (**91/91 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (91/91). Depth:
