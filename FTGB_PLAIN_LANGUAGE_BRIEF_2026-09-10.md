...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **89/89 PASS**.
...
All of it re-runs from one command (**89/89 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (89/89). Depth:
