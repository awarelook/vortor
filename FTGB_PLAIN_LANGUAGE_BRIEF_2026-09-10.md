...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **71/71 PASS**.
...
All of it re-runs from one command (**71/71 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (71/71). Depth:
