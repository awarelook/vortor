...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **65/65 PASS**.
...
All of it re-runs from one command (**65/65 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (65/65). Depth:
