...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **59/59 PASS**.
...
All of it re-runs from one command (**59/59 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (59/59). Depth:
