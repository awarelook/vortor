...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **45/45 PASS**.
...
All of it re-runs from one command (**45/45 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (45/45). Depth:
