...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **84/84 PASS**.
...
All of it re-runs from one command (**84/84 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (84/84). Depth:
