...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **77/77 PASS**.
...
All of it re-runs from one command (**77/77 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (77/77). Depth:
