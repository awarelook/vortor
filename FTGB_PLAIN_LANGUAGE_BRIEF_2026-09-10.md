...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **58/58 PASS**.
...
All of it re-runs from one command (**58/58 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (58/58). Depth:
