...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **69/69 PASS**.
...
All of it re-runs from one command (**69/69 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (69/69). Depth:
