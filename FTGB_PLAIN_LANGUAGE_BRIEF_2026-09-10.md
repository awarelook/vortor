...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **86/86 PASS**.
...
All of it re-runs from one command (**86/86 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (86/86). Depth:
