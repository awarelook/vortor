...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **85/85 PASS**.
...
All of it re-runs from one command (**85/85 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (85/85). Depth:
