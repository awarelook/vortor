...
`pip install -r requirements.txt && python results/verify/verify_all.py` → **76/76 PASS**.
...
All of it re-runs from one command (**76/76 checks pass**), with no internet and no hidden fudge.
...
*Reproduce: `pip install -r requirements.txt && python results/verify/verify_all.py` (76/76). Depth:
