...
*Reproducibility gate: every push runs the full model (`verify_all.py`, **59/59** checks) on a clean
...
| `requirements.txt` | Python deps (mpmath, numpy) — **`pip install -r requirements.txt && python results/verify/verify_all.py`** re-runs the whole model: **59/59** checks (56 theory scripts + the engine + the synthesis modeler + the resonator simulation), exit 0 iff every one passes |
