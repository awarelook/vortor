...
| `requirements.txt` | Python deps (mpmath, numpy) — **`pip install -r requirements.txt && python results/verify/verify_all.py`** re-runs the whole model: **40/40** checks (38 theory scripts + the engine + the synthesis modeler), exit 0 iff every one passes |
