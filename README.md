...
| `requirements.txt` | Python deps (mpmath, numpy) — **`pip install -r requirements.txt && python results/verify/verify_all.py`** re-runs the whole model: **41/41** checks (39 theory scripts + the engine + the synthesis modeler), exit 0 iff every one passes |
