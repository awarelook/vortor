...
*Reproducibility gate: every push runs the full model (`verify_all.py`, **49/49** checks) on a clean
...
| `requirements.txt` | Python deps (mpmath, numpy) — **`pip install -r requirements.txt && python results/verify/verify_all.py`** re-runs the whole model: **49/49** checks (47 theory scripts + the engine + the synthesis modeler), exit 0 iff every one passes |
