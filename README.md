...
*Reproducibility gate: every push runs the full model (`verify_all.py`, **65/65** checks) on a clean
...
| `MATH_TOOLKIT_BASE.md` | **the shared foundation** the nine modules rest on — Beltrami/CK curl eigenproblem (§1), helicity/Woltjer-Taylor (§4), the *one operator → three readings* (plasma ‖ Madelung matter-wave ‖ `K_PV` vacuum) + topology (§9); honesty-tiered, numerology quarantined |
...
| `requirements.txt` | Python deps (mpmath, numpy) — **`pip install -r requirements.txt && python results/verify/verify_all.py`** re-runs the whole model: **65/65** checks (62 theory scripts + the engine + the synthesis modeler + the resonator simulation), exit 0 iff every one passes |
