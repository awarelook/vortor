...
*Reproducibility gate: every push runs the full model (`verify_all.py`, **71/71** checks) on a clean
...
| `requirements.txt` | Python deps (mpmath, numpy) — **`pip install -r requirements.txt && python results/verify/verify_all.py`** re-runs the whole model: **71/71** checks (68 theory scripts + the engine + the synthesis modeler + the resonator simulation), exit 0 iff every one passes |
