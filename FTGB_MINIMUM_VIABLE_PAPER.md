...
and a matter wave. Author: Nathaniel Hanks · v1.1 · reproduce: `python results/verify/verify_all.py` (**60/60**,
...
| **Driven / sustained** | forcing `f=νλ²u_B` makes it an **exact steady solution** (self-sustaining) and a **stable attractor** (a 30% perturbation decays; full nonlinear NS, Re≈157); a Taylor–Green base drifts | `[V]` + `[V@Re]` `r2_driven_beltrami_attractor_check` |
...
python results/verify/verify_all.py      # 60/60 PASS, deterministic, no network, CI-gated
```
